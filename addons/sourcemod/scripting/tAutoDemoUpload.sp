...
public OnPluginStart() {
	CreateConVar("sm_tautodemoupload_version", VERSION, "Uploads demo files to a remote ftp server", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);

	g_hCvarEnabled = CreateConVar("sm_tautodemoupload_enable", "1", "Automatically upload demos when finished recording.", _, true, 0.0, true, 1.0);
	HookConVarChange(g_hCvarEnabled, Cvar_Changed);

	g_hCvarBzip = CreateConVar("sm_tautodemoupload_bzip2", "9", "Compression level. If set > 0 demos will be compressed before uploading. (Requires bzip2 extension.)", _, true, 0.0, true, 9.0);
	HookConVarChange(g_hCvarBzip, Cvar_Changed);

	g_hCvarDelete = CreateConVar("sm_tautodemoupload_delete", "0", "Delete the demo (and the bz2) if upload was successful.", _, true, 0.0, true, 1.0);
	HookConVarChange(g_hCvarDelete, Cvar_Changed);

	g_hCvarFtpTarget = CreateConVar("sm_tautodemoupload_ftptarget", "demos", "The ftp target to use for uploads.");
